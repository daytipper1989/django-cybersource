...
class PaymentForm(forms.Form):
    ...
    signed_field_names = forms.CharField(label=_('Signed Field Names'), required=True, initial = "access_key,profile_id,transaction_uuid,signed_field_names,unsigned_field_names,signed_date_time,locale,transaction_type,reference_number,amount,currency,bill_address1,bill_city,bill_country,customer_email,customer_lastname", disabled = True,widget = forms.HiddenInput())
    unsigned_field_names = forms.CharField(label=_('Unsigned Field Names'), required=False, initial = "signature", disabled = True,widget = forms.HiddenInput())
