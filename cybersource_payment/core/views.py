...
def confirm(request):
	...
	if request.method == 'POST':
		...
		if paymentForm.is_valid():
			...
			keys = request.POST['signed_field_names'].split(',')
			message = ','.join(['{key}={value}'.format(key=key, value=paramsDict[key]) for key in keys])
			messageBeforeEncoding = message
			message = message.encode('utf-8')
			signature = base64.b64encode(hmac.new(secretKey.encode('utf-8'), msg=message, digestmod=hashlib.sha256).digest()).decode()
